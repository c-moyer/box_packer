    ...
    def pack2(containers:, items:)
      ...
      items.each do |item|
        ...
        packings.each do |packing|
          # If this packings going to be too big with this
          # item as well then skip on to the next packing
          next if packing[:weight].to_f + item[:weight].to_f > packing[:weight_limit].to_f
        ...
        # Can't fit in any of the spaces for the current packings
        #loop through containers and find best fit
        placement = nil
        space = nil
        weight_limit = nil
        containers.each do |c|
          next unless item[:weight].to_f <= c[:weight_limit].to_f
          space = {
            dimensions: c[:dimensions].sort.reverse,
            position: [0, 0, 0]
          }
          weight_limit = c[:weight_limit].to_f
          placement = place(item, space)
